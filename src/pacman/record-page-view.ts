...
export default function(d: Document, w: Window, n: Navigator, pacman: IPacman) {
  ...
  var r = function(f) {
    ...
    pv = {
      ...
      lr: r(function() {
        return (
          w.screen.width < w.screen.availWidth ||
          w.screen.height < w.screen.availHeight
        );
      }),
      lo: r(function() {
        var cpu = n.oscpu,
          p = n.platform.toLowerCase();
        if (ua.indexOf("windows phone") >= 0 || ua.indexOf("win") >= 0)
          return null;
        if (ua.indexOf("android") >= 0)
          return (
            typeof n.plugins === "undefined" ||
            p.indexOf("win") >= 0 ||
            p.indexOf("mac") >= 0 ||
            p.indexOf("ipad") >= 0 ||
            p.indexOf("ipod") >= 0 ||
