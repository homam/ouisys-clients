...
import recordPacmanPageView from "./record-page-view";
import recordPacmanInteractions from "./main";
...
export function recordImpression(
  ...
  if (!window.pac_analytics) {
    ...
    window.pac_analytics.queryString = key =>
      queryString(window.location.search, key);
  }
...
export default (window: Window, country: string, page: string): ITracker => {
  if (!window) {
    return {
      viewChanged: () => void 6,
