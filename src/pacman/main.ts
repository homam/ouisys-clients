...
export default function(
  ...
  var documentProp = {
    ...
    getViewportSize: function() {
      ...
      return [
        rootElement.clientWidth || w.innerWidth,
        rootElement.clientHeight || w.innerHeight
      ];
    },
  ...
  var eventsManager = {
    getPos: function(event) {
      ...
      return [
        event.pageX || event.clientX + c[0] - (b.clientLeft || 0) || 0,
        event.pageY || event.clientY + c[1] - (b.clientTop || 0) || 0
      ];
    },
  ...
  if (visibilityProperty in d) visibilityChangeEvent = "visibilitychange";
  ...
  var clicks = zip(touchstart, touchend).pipe(
    filter(function(x) {
      return (
  ...
  var stream = merge(
    ...
    bufferWhen(function() {
      return merge(interval(7000), unload, blur);
    }),
    filter(function(x) {
      return x.length > 0;
    })
